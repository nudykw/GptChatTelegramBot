...
Проєкт базується на **.NET 10**. Залежно від вашого сценарію, оберіть зручний спосіб запуску:

| Спосіб | Опис |
|---|---|
| **🖥️ Локально** (`dotnet run`) | Найпростіше для розробки. Консоль (polling) або веб (polling/webhook). |
| **🐳 Docker Compose** | Рекомендовано для продакшну. Веб-версія + БД + CloudBeaver. |
| **🐳 Docker (консоль)** | Легкий контейнер без HTTP-сервера, тільки polling. |
| **🖥️ systemd (на Linux VPS)** | Без Docker як системний сервіс. |

📚 Покрокові інструкції для кожного способу: [Хостинг та способи запуску](docs/HOSTING.uk.md).
...
Бот підтримує кілька способів запуску: локально, Docker Compose (рекомендовано) та як systemd-сервіс.

Повний посібник: [Хостинг та способи запуску](docs/HOSTING.uk.md)
